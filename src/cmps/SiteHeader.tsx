...
export function SiteHeader() {
  ...
  const cvHref = profile.cvPath ? `${import.meta.env.BASE_URL}${profile.cvPath.replace(/^\//, '')}` : ''
  ...
  return (
    <header className="site-header">
      ...
      <nav className="site-header-nav" aria-label={t('nav.site')}>
        ...
        <div className="site-header-lang" role="group" aria-label={t('action.language')}>
        ...
        {cvHref ? (
          <a className="site-header-cv" href={cvHref} title={t('action.cv')} download>
            {t('action.cv')}
          </a>
        ) : null}
      </nav>
